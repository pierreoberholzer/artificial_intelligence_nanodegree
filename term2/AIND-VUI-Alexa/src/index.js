...
var handlers = {
    ...
    'GetFact': function () {
        ...
        var reask = 'Would you like another fact?';

        this.emit(':askWithCard', speechOutput, reask, this.t("SKILL_NAME"), randomFact);
    },
    ...
    'GetYearFact': function () {
        ...
        //var year = this.event.request.intent.slots["object.FACT_YEAR"].value;
        //var year = 1998;
        //var year = this.FACT_YEAR;
        var factArr = this.t('FACTS');
        var factArr_valid = factArr.filter(function(s){
            //return s.includes(year);
            s.indexOf(year) != -1;
        });

        //if (typeof factArr_valid !== 'undefined' && factArr_valid.length > 0) {
        if (factArr_valid.length > 0) {
                // the array is defined and has at least one element
            var randomFact = randomPhrase(factArr_valid);
            var speechOutput = this.t("GET_FACT_MESSAGE") + randomFact;
        } else {
            var randomFact = randomPhrase(factArr);
            var speechOutput = this.t("GET_FACT_MESSAGE")+ "Nothing was found in year "+year+". I give you info for another year." + randomFact;
        }
