...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if len (set(node_a1.action.effect_add).intersection(set(node_a2.action.effect_rem))) or \
           len (set(node_a2.action.effect_add).intersection(set(node_a1.action.effect_rem))) :
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if len(set(node_a1.action.effect_add).intersection(set(node_a2.action.precond_neg))) or \
           len(set(node_a1.action.effect_rem).intersection(set(node_a2.action.precond_pos))) or \
           len(set(node_a2.action.effect_add).intersection(set(node_a1.action.precond_neg))) or \
           len(set(node_a2.action.effect_rem).intersection(set(node_a1.action.precond_pos))):
            return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        # Calculates cost for each subgoal and return "inf" if no subgoal is found
        def cost_per_goal(self,g,level_sum):
        ...
        # Iterates over all subgoals and cumulates cost
        for g in self.problem.goal:
            level_sum += cost_per_goal(self,g,level_sum)
            if level_sum == float('inf'):
                break
