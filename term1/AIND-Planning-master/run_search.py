...
def run_search(pname,sname,hstring,problem, search_function, parameter=None):
    ...
    # My stuff: writing results to csv
    res = {}
    res['Timestamp'] = time.strftime("%d:%m:%Y %H:%M:%S")
    res['Problem'] = pname
    res['Search'] = sname
    res['Heuristic'] = hstring
    res['Plan length'] = len(node.solution())
    res['Time elapsed'] = end-start
    res['Expansions'] = ip.get_res()[0]
    res['Goal tests'] = ip.get_res()[1]
    res['New nodes'] = ip.get_res()[2]

    #res = [time.strftime("%d:%m:%Y %H:%M:%S")] + [pname,sname,hstring]+[len(node.solution()), end-start]+ip.get_res()

    filename = "res.csv"

    headers = ["Timestamp", "Problem", "Search", "Heuristic", "Plan length", "Time elapsed", "Expansions", "Goal tests",
               "New nodes"]
...
def main(p_choices, s_choices):
    ...
    for pname, p in problems:
        ...
        for sname, s, h in searches:
            ...
            #run_search(_p, s, _h)
            run_search(pname,sname,hstring,_p, s, _h)
